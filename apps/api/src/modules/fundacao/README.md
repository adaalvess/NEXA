...
Passo 9 estendeu o `AuthorizationService` com `obterRelacaoEntidade`/`podeAgirSobreEntidade`/`obterEscopoVisibilidade` (visibilidade RBAC centralizada, consumida pelo módulo `processos` e pelo `PartilhaService`, refatorado para as usar). `AuthorizationService`, `PermissaoGuard` e `SessionGuard` são agora exportados por este módulo, para consumo por qualquer módulo de negócio (Processos, e os seguintes CRM/Dashboard).

Ver: Blueprint de Implementação (secção 3, 3a, 4), Especificações Técnicas dos Passos 3 a 9, ADR-001, ADR-003, ADR-004.
