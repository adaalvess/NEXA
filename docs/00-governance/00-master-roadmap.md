...
| 7 | Desenvolvimento da Plataforma (código) | 🔄 Em curso — M1 (Fundação) formalmente concluído (Passos 0-7); M2 (Módulos Core) em curso, Passos 8-10 concluídos |
...
| 7 — Desenvolvimento | 🔄 **Em curso** — M1 (Fundação) do Blueprint **formalmente concluído** (Passos 0-7: scaffolding, schema Prisma + migração, Autenticação, Camada 1/RLS, RBAC granular, Registo de Auditoria, Partilha/Convidado). **M2 (Módulos Core) aprovado e em curso** — Passo 8 (Departamento), Passo 9 (Processos/Tarefas, primeiro módulo de negócio fora da Fundação) e Passo 10 (CRM, segundo módulo, zero alterações ao `AuthorizationService`) concluídos; Passo 11 (Notification Dispatcher) por iniciar. Exit criteria da Fase 7 como um todo (todos os FR/US/UC do MVP implementados, testes dos 4 fluxos críticos) continua por cumprir — o M1 é só o primeiro de vários milestones do Blueprint dentro desta fase |
...
| **M8** | Início do desenvolvimento (Fase 7, Claude Code) | 🔄 Em curso — Blueprint M1 (Fundação) formalmente concluído (Passos 0-7); M2 (Módulos Core) em curso, Passos 8-10 concluídos |
...
**Milestone M1 (Fundação) formalmente concluído (2026-07-06, Passo 7). Milestone M2 (Módulos Core) aprovado e em curso — Passo 8 (Departamento), Passo 9 (Processos/Tarefas) e Passo 10 (CRM) concluídos e formalmente aprovados, ver 3.8/3.9/3.10 do CLAUDE.md e as respetivas Especificações Técnicas. Passo 9 centralizou a lógica de visibilidade RBAC no `AuthorizationService` (Decisão B do M2); Passo 10 confirmou na prática que essa centralização funciona — zero alterações ao `AuthorizationService` foram necessárias para o segundo módulo de negócio. Próximo: Passo 11 (Notification Dispatcher), primeiro consumidor de eventos fire-and-forget do projeto.**
