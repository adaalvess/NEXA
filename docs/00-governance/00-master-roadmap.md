...
| 7 | Desenvolvimento da Plataforma (código) | 🔄 Em curso — M1 (Fundação) formalmente concluído (Passos 0-7); M2 (Módulos Core) em curso, Passos 8-11 concluídos |
...
| 7 — Desenvolvimento | 🔄 **Em curso** — M1 (Fundação) do Blueprint **formalmente concluído** (Passos 0-7: scaffolding, schema Prisma + migração, Autenticação, Camada 1/RLS, RBAC granular, Registo de Auditoria, Partilha/Convidado). **M2 (Módulos Core) aprovado e em curso** — Passo 8 (Departamento), Passo 9 (Processos/Tarefas, primeiro módulo de negócio fora da Fundação), Passo 10 (CRM, segundo módulo, zero alterações ao `AuthorizationService`) e Passo 11 (Notification Dispatcher, primeiro consumidor fire-and-forget) concluídos; Passo 12 (Dashboard) por iniciar. Exit criteria da Fase 7 como um todo (todos os FR/US/UC do MVP implementados, testes dos 4 fluxos críticos) continua por cumprir — o M1 é só o primeiro de vários milestones do Blueprint dentro desta fase |
...
| **M8** | Início do desenvolvimento (Fase 7, Claude Code) | 🔄 Em curso — Blueprint M1 (Fundação) formalmente concluído (Passos 0-7); M2 (Módulos Core) em curso, Passos 8-11 concluídos |
...
**Milestone M1 (Fundação) formalmente concluído (2026-07-06, Passo 7). Milestone M2 (Módulos Core) aprovado e em curso — Passos 8 a 11 concluídos e formalmente aprovados, ver 3.8-3.11 do CLAUDE.md e as respetivas Especificações Técnicas. Passo 9 centralizou a lógica de visibilidade RBAC no `AuthorizationService` (Decisão B do M2); Passo 10 confirmou na prática que essa centralização funciona sem duplicação; Passo 11 introduziu o primeiro consumidor de eventos fire-and-forget (`NotificacaoListener`), reutilizando o `EVENTO_AUDITORIA` já existente sem alterar nenhum ponto de emissão. Próximo: Passo 12 (Dashboard), primeiro consumidor real da tabela `Notificacao`.**
